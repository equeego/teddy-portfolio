  ...
  const renderElement = (element: any) => (
    <Link
      ...
      <div className="p-6">
        ...
        <p className="font-semibold capitalize text-sm">{t(translate["read-more"])}</p>
      </div>
    </Link>
  );
    ...
    <section
      ...
      <div className="mx-auto md:px-24 container">
        <div className="text-center mb-10 md:mb-20 md:mx-auto text-white">
          ...
          {titles.subtitle && (
            <p className="max-w-xl text-base md:text-lg mx-auto">{t(titles.subtitle)}</p>
          )}
        </div>
        <div
          className={`rounded-xl overflow-hidden grid gap-x-0 grid-cols-1 lg:grid-cols-${
            data.length > 1 ? "2" : "1"
          }`}>
          {data.length > 0 &&
            (data.length === 1
              ...
              : renderMainElement(data[0]))}

          {data.length > 1 && (
            <div>{data.slice(1).map((aboutElement: any) => renderElement(aboutElement))}</div>
          )}
        </div>
      </div>
    </section>
