    ...
    <section className="px-6 py-10 md:py-20">
      <div className="mx-auto md:px-24 container">
        ...
        <div className={`grid gap-6 lg:grid-cols-${data.length}`}>
          {data.map((member: any) => {
            const trimmedContent = _.flatMap(t(member.content) || [], 'children').map(child => child.text).join(' ');
            return (
              <Link href={`team/${t(member.slug)}`} prefetch={false} key={`hometeam--${member._id}`} className="sm:text-center max-w-[300px] mx-auto hover:scale-110 duration-700 transition-all drop-shadow-md">
                <div className="flex items-center justify-center w-16 h-16 mb-4 rounded-full sm:mx-auto sm:w-24 sm:h-24 overflow-hidden">
                  <Image src={member.image} width={200} height={200} alt={member.image_alt || " "} style={{ height: 'calc(100% - 1px)', width: '100%' }} />
                </div>
                <h6 className="mb-2 font-semibold leading-5">{t(member.name)}</h6>
                <p className="max-w-md mb-3 text-sm text-gray-900 sm:mx-auto min-h-[40px]">{t(member.description)}</p>
